...
```
select municipality, count(freguesia) as num_citrus_pl from perm_crop_freg where `hold` > 0 and crop_name = "Citrus plantations" group by municipality;
```
...
```
select municipality, sum(area) as sum_area_olive from perm_crop_freg where crop_name like "Olive%" group by municipality;
```
